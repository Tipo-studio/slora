...
function JoinBetaPage({ onBack, user, onRequestLogin, ...headerProps }: JoinBetaPageProps) {
  ...
  return <main className="join-beta-page" data-figma-node-id="11798:1773">
    ...
    <Popup open={isSuccessPopupOpen} title="Join Beta Success" onClose={() => { setIsSuccessPopupOpen(false); window.dispatchEvent(new Event('join-beta-submitted')) }} icon={<CheckCircle2 size={48} strokeWidth={1.5} color="#10b981" />} className="app-popup-template join-beta-success-popup">
      <div className="app-popup-art home2-promo-art" aria-hidden="true"><img src="/images/reward-code-banner.png" alt="" /></div>
      <div className="app-popup-form home2-promo-form join-beta-success-content"><p>{message}</p><button type="button" onClick={() => { setIsSuccessPopupOpen(false); window.dispatchEvent(new Event('join-beta-submitted')); onBack() }}>START CREATING</button></div>
    </Popup>
  </main>
}
