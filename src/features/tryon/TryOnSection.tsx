...
import { ChevronLeft, ChevronRight, Download, Maximize2, Pencil, Shirt } from 'lucide-react'
...
function TryOnSection({ sectionRef, user, onRequestLogin, onOpenAccount, onOpenPaywall, initialTool, initialImageUrl }: { sectionRef: RefObject<HTMLElement | null>; user: User | null; onRequestLogin: () => void; onOpenAccount: () => void; onOpenPaywall: (plan: 'one-time' | 'creator' | 'studio', returnToResult?: boolean) => void; initialTool?: 'try-on' | 'magic-editor'; initialImageUrl?: string | null }) {
  ...
  return <section ref={sectionRef} id="tryon" className="tryon-screen relative z-10 min-h-screen" aria-label="Try-on tools">
    ...
    <Popup open={generationBalanceUpdate !== null} title="Generation balance updated" titleId="tryon-generation-balance-title" eyebrow="Generation updated" icon={<GenerationIcon size={28} />} description="Your available generation balance has changed." onClose={() => setGenerationBalanceUpdate(null)} className="app-popup-template home2-generation-popup">
    ...
    <Popup open={isFullPreviewOpen && Boolean(resultOutput?.url)} title="Full image preview" onClose={() => setIsFullPreviewOpen(false)} className="app-popup-template tryon-full-preview-popup">
      {resultOutput?.url && <div className="tryon-full-preview-content"><img src={resultOutput.url} alt="Generated result full preview" /></div>}
    </Popup>
  </section>
}
