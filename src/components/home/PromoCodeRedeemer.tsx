...
function PromoCodeRedeemer({ onRedeemed, onTryNow }: { onRedeemed: (update: { granted: number; remaining: number }) => void; onTryNow: () => void }) {
  const inputId = useId()
  ...
  return <>
    ...
    <Popup open={isOpen} title="promotion code" titleId="home2-promo-title" onClose={close} closeDisabled={isSubmitting} className="app-popup-template home2-promo-popup">
        <div className="home2-promo-art" aria-hidden="true"><img src="/images/reward-code-banner.png" alt="" /></div>
        <div className={`home2-promo-form ${message ? 'has-feedback' : ''}`}>
          {!message && <>
            <label htmlFor={inputId}>referral code</label>
            <input id={inputId} value={code} onChange={(event) => { setCode(event.target.value.toUpperCase()); setMessage('') }} onKeyDown={(event) => { if (event.key === 'Enter') { event.preventDefault(); void submit() } }} placeholder="code" maxLength={40} autoComplete="off" disabled={isSubmitting} />
            <p>Enter your referral code to add free generations to your account.</p>
            <button type="button" onClick={() => void submit()} disabled={isSubmitting}>{isSubmitting ? 'APPLYING…' : 'APPLY'}</button>
          </>}
          {message && <div className={`home2-promo-feedback ${isRedeemed ? 'is-success' : 'is-error'}`} role="status"><strong>{isRedeemed ? 'Redeem successful' : 'Unable to redeem code'}</strong><span>{message}</span><button type="button" className="home2-promo-try-now" onClick={() => { setIsOpen(false); onTryNow() }}>TRY NOW</button></div>}
        </div>
    </Popup>
  </>
}
